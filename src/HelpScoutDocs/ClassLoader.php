...
final class ClassLoader {
	...
	public function autoload($className) {
		if (strpos($className, 'HelpScoutDocs') === false) {
			return false;
		}
		$className = str_replace(
				array(
						self::NAMESPACE_SEPARATOR . 'HelpScoutDocs' . self::NAMESPACE_SEPARATOR,
						'HelpScoutDocs' . self::NAMESPACE_SEPARATOR
				), '', $className
		);
